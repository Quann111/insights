	...
	return {
		...
		xAxis: {
			...
			axisLabel: {
				...
				fontSize: 14, // tương đương với text-lg trong Tailwind
				fontWeight: '500', // tương đương với font-medium trong Tailwind
				lineHeight: 24, 
				formatter: (value) => {
		...
		series: datasets.map((dataset, index) => ({
			...
			// barWidth: '20%',  // Adjust bar width here, e.g., '20px' or '20%'
            // barGap: '30%',    // Adjust space between bars here, e.g., '30%'
		...
		legend: {
			...
			pageButtonItemGap: 2,
			textStyle: {  // Add this textStyle property to modify legend text
				fontSize: 12, // Smaller font size for legend items
				fontFamily: 'Segoe UI', // Optional: set a specific font
				fontWeight: 'normal' // Less emphasis on legend text
			}	
		},
		tooltip: {
			trigger: 'axis',
			confine: true,
			appendToBody: false,
			// Không làm tròn giá trị trong tooltip, hiển thị 2 chữ số thập phân
